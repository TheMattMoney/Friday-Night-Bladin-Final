...
class PlayState extends MusicBeatState
{
	...
	if(Storage.startingTime>0)
		{
			...
			songScore =Storage.songScore;
		   songMisses =Storage.songMisses;
	...
	public function startCountdown():Void
	{
		...
		if(ret != FunkinLua.Function_Stop) {
			if (skipCountdown || Storage.startingTime > 0) skipArrowStartTween = true;
			...
			if (skipCountdown || Storage.startingTime > 0) {
				clearNotesBefore(Storage.checkPoint);
				setSongTime(Storage.checkPoint - 500);
				return;
			}
	...
	public function clearNotesBefore(time:Float)
		...
		function checkPointCheck()
			{
				if(300000<FlxG.sound.music.time)
					Storage.checkPoint=0;
				if(300000>=FlxG.sound.music.time)
					Storage.checkPoint=300000;
				if(600000>=FlxG.sound.music.time)
					Storage.checkPoint=600000;
				if(900000>=FlxG.sound.music.time)
					Storage.checkPoint=900000;
				if(300000>=FlxG.sound.music.time)
					{
						Storage.songScore=songScore;
						Storage.songMisses=songMisses;
						Storage.ratingName=ratingName;
						Storage.ratingPercent=ratingPercent;
					}
			}
...
function hittingSpaceSHit()
	{
		...
		if(pressedSpaceCounter>=2)
			{
				trace('YOU SPAMMED SPACE');
				doDamage=true;
	...
	public static var vibratin=true;
	override public function update(elapsed:Float)
	{
		...
		if(boyfriend.animation.curAnim.name.startsWith('sing'))	
   	   ...
   	   if(beysAlert.alpha==1)
		hittingSpaceSHit();
		if(doDamage ==true&&beysAlert.alpha==0)
			{
		...
		iconP2.y = ((healthBar.height * (FlxMath.remapToRange(healthBar.percent, 0, 100, 100, 0) * 0.01)) - (150 * iconP2.scale.y) / 2 - iconOffset * 2)+adj ;
		...
		FlxG.watch.addQuick("CheckPoint in Storage", Storage.startingTime);
   ...
   var lightanimfar:Int=0;
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String) {
		switch(eventName) {
		    ...
		    case "beyShowDown":
					...
					FlxTween.tween(beysglow, {alpha: 1}, 0.5,{onComplete: function(twn:FlxTween)
						{
							beysAlert.alpha=1;
						
							new FlxTimer().start(0.8, function(tmr:FlxTimer)//actual time the player has
								{
									beysAlert.alpha=0;
									FlxTween.tween(beysglow, {alpha: 0}, 0.000001); ////oh my fucking GOD
									new FlxTimer().start(0.15, function(tmr:FlxTimer){camGame.shake(0.009, 0.1);});
    ...
    var healthMeterLoss:FlxTimer;
	var healthMeterLoss2:FlxTimer;
    var healthMeterLoss3:FlxTimer;
	var healthMeterLossSpeed:Float=0.35;
	...
	override function stepHit()
	{
        ...
        if(SONG.notes[Math.floor(curStep / 16)].mustHitSection ==true||boyfriend.animation.curAnim.name.startsWith('sing'))
			defaultCamZoom=0.85;
		else
			defaultCamZoom=1;
			...
			if(Storage.startingTime>0)
				{
					...
					ratingName =Storage.ratingName;
					ratingPercent =Storage.ratingPercent;
